...
4. Update report UI in `c:\BTR\OpenSource\tally\src\tally\spending_report.js` and `c:\BTR\OpenSource\tally\src\tally\spending_report.html`:
   ...
   - **Footers count distinct merchants; rows stay composite.** The composite row is the correct budget unit — "Amazon in Health/Supplements, $12.83/mo" and "Amazon in Pet/Food, $2.44/mo" are two budget lines that happen to share a payee, and merging them into one "Amazon" row makes the audit less actionable because you cannot tag or act on "Amazon" as a whole. Only the summary footers group by payee:
     - Chart caption (`spending_report.js:4828-4835`): dedupe `fixedNames` by `merchant` display name before `slice(0, 10)`, summing `recurringMonthlyCost` across a name's composite rows for ranking. `+ N more` then counts distinct names in the tail. This footer is names-only, so grouping is unambiguous. Preserve the existing rank-by-cost behavior from `c45fc76` — do not re-sort alphabetically.
     - Audit total row (`spending_report.js:4998`): `${rows.length} recurring merchants` currently counts composite rows, not merchants. Change to distinct-merchant count, and state both so the number reconciles with the visible table — e.g. `M merchants · N rows`.
     - Leave the audit's `+ N more` (`spending_report.js:4978-4991`) as a **row** count. Its adjacent dollar figure is a sum over hidden rows, so a distinct-merchant count there would describe a different population than the money next to it — and a merchant with one visible and one hidden row belongs to both halves. Row count is the only reading that stays coherent.
   - Sequencing: do the footer work **after** step 2. In current real data 70 of 82 chart-eligible fixed rows come from the `infer_annual_amount()` bug, no merchant carries the `fixed` tag at all (the personal config uses `fixed-budget`/`fixed-annual`, which `analyzer.py` ignores), and every duplicate-name collision — Amazon×4, Target×2, Kwik Trip×2 — ranks below the top 10. Grouping polishes a population that step 2 and step 8 replace.
...
7. Update tests:
   ...
   - Add a footer test: a merchant with multiple composite rows must appear once in the chart caption's `Top 10 Fixed` list, and the audit total must report distinct merchants while `+ N more` still reports rows.
...
- Do not merge composite audit rows by payee. Merchant grouping was raised after `Top 10 Fixed` displayed "Amazon, Amazon, Amazon" — that repetition was entirely the alphabetical-sort bug fixed in `c45fc76`, not a grouping defect. Under the corrected rank-by-cost the top 12 are already twelve distinct names. Grouping is therefore a footer-labeling concern only; the underlying row identity stays composite. If merged rows are ever revisited, the shape would be per-payment-stream category/cadence/monthly with a summed annualized total — but nothing in current data motivates it.
